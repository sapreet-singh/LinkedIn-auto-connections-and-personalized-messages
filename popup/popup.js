const CONSTANTS = {
  STEPS: {
    CAMPAIGN_NAME: 1,
    SOURCE_SELECTION: 2,
    PROFILE_COLLECTION: 3,
    MESSAGING: 4,
  },
  SUBSTEPS: { SEARCH: "search", NETWORK: "network", COLLECTING: "collecting" },
  URLS: {
    NETWORK_SEARCH:
      "https://www.linkedin.com/search/results/people/?network=%5B%22F%22%5D&origin=FACETED_SEARCH",
    CONNECTIONS:
    ...
    try {
      ...
      if (profiles.length === 0) {
        Utils.showNotification(
          "No profiles available for message generation",
          "warning"
        );
        return;
      }
      ...
      Utils.showNotification(
        `Generated ${successCount} personalized messages`,
        "success"
      );
    } catch (error) {
      console.error("Message generation failed:", error);
      Utils.showNotification(
        "Failed to generate messages. Please check your API connection.",
        "error"
      );
      statusDiv.style.display = "none";
    } finally {
    ...
    results.forEach((result) => {
      ...
      if (result.success) {
        ...
        messageItem.innerHTML = `
                    ...
                    ${
                      followUpMessage
                        ? `<div class="message-content" style="margin-top: 8px; border-left-color: #28a745;"><strong>Follow-up:</strong> ${followUpMessage}</div>`
                        : ""
                    }
            ...
            <div class="profile-pic">
                ${
                  profilePic
                    ...
                    : `<div class="profile-avatar" style="width: 50px; height: 50px; border-radius: 50%; background: #0073b1; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 18px;">${cleanName
                        .charAt(0)
                        .toUpperCase()}</div>`
                }
            </div>
            <div class="profile-info">
                ...
                <div class="profile-details" style="color: #666; font-size: 12px;">
                    ${
                      title && company
                        ? `${title} at ${company}`
                        : title || company || "• 1st degree connection"
                    }
                </div>
                ${
                  url
                    ? `<div class="profile-url" style="color: #0073b1; font-size: 11px; word-break: break-all;">${url}</div>`
                    : ""
                }
            </div>
        `;
...
const ModalManager = {
  init() {
    ...
    DOMCache.getAll(".close").forEach((btn) =>
      btn.addEventListener("click", (e) => this.handleCloseClick(e))
    );
    DOMCache.get("close-profiles")?.addEventListener("click", () =>
      this.closeModal("profiles-modal")
    );
  ...
  handleCloseClick(e) {
    ...
    e.stopPropagation();
    Utils.showNotification(
      "Close button is disabled. Modal will remain open.",
      "info"
    );
    return false;
  },
  ...
  closeModal(modalIdOrEvent) {
    const modalId =
      typeof modalIdOrEvent === "string"
        ? modalIdOrEvent
        : modalIdOrEvent.target.closest("#campaign-modal")
        ? "campaign-modal"
        : null;
    if (modalId) {
  ...
  forceCloseAll() {
    ["campaign-modal", "profiles-modal", "profile-urls-modal"].forEach(
      (modalId) => {
        Utils.hideById(modalId);
      }
    );
...
const WizardManager = {
  ...
  setupEventListeners() {
    const eventMap = {
      ...
      "show-filters": () =>
        chrome.tabs.create({ url: CONSTANTS.URLS.PEOPLE_SEARCH }),
      "start-collecting": () => {
        this.showStep(3, "collecting");
        ProfileCollector.start();
      },
      "start-multi-page-collecting": () => {
        this.showStep(3, "collecting");
        ProfileCollector.startMultiPage();
      },
      "show-network-filters": () => NetworkManager.openSearch(),
      "start-network-collecting": () => {
        this.showStep(3, "collecting");
        NetworkManager.startCollecting();
      },
      "start-network-multi-page-collecting": () => {
        this.showStep(3, "collecting");
        NetworkManager.startMultiPageCollecting();
      },
      "browse-connections": () => NetworkManager.browseConnections(),
      "show-sales-navigator-filters": () => SalesNavigatorManager.openSearch(),
      "start-sales-navigator-collecting": () => {
        this.showStep(3, "collecting");
        SalesNavigatorManager.startCollecting();
      },
...
const RealTimeProfileHandler = {
  init() {
    chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
      if (message.action === "addProfilesRealTime" && message.profiles) {
        ...
        message.action === "collectionStatusUpdate" &&
        message.message
      ) {
  ...
  handleRealTimeProfiles(profiles) {
    ...
    if (validProfiles.length > 0) {
      const newProfiles = validProfiles.filter((newProfile) => {
        return !AppState.collectedProfiles.some(
          (existingProfile) => existingProfile.url === newProfile.url
        );
      });
  ...
  updateUIAfterModalOpen(newProfileCount) {
    ...
    Utils.showNotification(
      `✅ Added ${newProfileCount} new profiles (Total: ${AppState.collectedProfiles.length})`,
      "success"
    );
  },
  ...
  handleCollectionStatus(statusMessage) {
    ...
    if (autoDetectionIndicator) {
      const statusSpan =
        autoDetectionIndicator.querySelector("span:last-child");
      if (statusSpan) {
    ...
    if (statusMessage.includes("complete") || statusMessage.includes("error")) {
      const notificationType = statusMessage.includes("error")
        ? "error"
        : "success";
...
const CampaignManager = {
  ...
  finalize() {
    ...
    this.load();
    Utils.showNotification(
      `Campaign "${campaignName}" created with ${AppState.collectedProfiles.length} profiles!`
    );
...
const ProfileManager = {
  view() {
    ...
    if (profiles.length === 0) {
      profilesList.innerHTML =
        '<div class="empty-state">No profiles collected yet</div>';
    } else {
  ...
  export() {
    ...
    const csvContent = [
      ...
      ...profiles.map(
        (p) => `"${p.name}","${p.url}","${p.company}","${p.title}"`
      ),
    ].join("\n");
  ...
  createCampaign() {
    ...
    DOMCache.get(
      "campaign-name"
    ).value = `Campaign from ${profiles.length} profiles`;
    WizardManager.showStep(3, "collecting");
    this.updateList();
    DOMCache.get("collected-number").textContent = profiles.length;
  },
...
const Step4Manager = {
  ...
  setupEventListeners() {
    ...
    if (useMessagesBtn)
      useMessagesBtn.addEventListener("click", () =>
        this.useSelectedMessages()
      );
    if (regenerateBtn)
      regenerateBtn.addEventListener("click", () => this.regenerateMessages());
    if (skipToBulkBtn)
      skipToBulkBtn.addEventListener("click", () => this.skipToBulkSend());
  },
  ...
  showProfileSelection() {
    ...
    AppState.collectedProfiles.forEach((profile, index) => {
      ...
      checkbox.addEventListener("change", () =>
        this.toggleProfile(index, checkbox.checked)
      );
  ...
  async generateMessages() {
    ...
    try {
        ...
        try {
          ...
          if (response) {
            if (typeof response === "string") {
              ...
              console.warn(
                "API response structure unclear for",
                profile.name,
      ...
      Utils.showNotification(
        `Generated ${this.generatedMessages.length} messages`,
        "success"
      );
    } catch (error) {
  ...
  showGeneratedMessages() {
    ...
    messagesContainer.innerHTML =
      '<div style="background: lime; padding: 20px; margin: 10px; border: 2px solid green; font-weight: bold;">🧪 TEST: Container is working! Messages should appear below...</div>';
      ...
      if (item.message.error) {
        ...
        if (messages.length === 0) {
          profileDiv.innerHTML = `
                        ...
                        <pre style="font-size: 11px; background: #f5f5f5; padding: 10px; margin: 10px; border-radius: 4px;">${JSON.stringify(
                          item.message,
                          null,
                          2
                        )}</pre>
  ...
  showSendMessageInterface() {
    ...
    const selectedProfiles = Step4Manager.selectedProfiles.map(
      (index) => AppState.collectedProfiles[index]
    );
  ...
  async startBulkMessageSending() {
    ...
    if (
      !Step4Manager.selectedProfiles ||
      Step4Manager.selectedProfiles.length === 0
    ) {
    ...
    try {
      ...
      for (let i = 0; i < Step4Manager.selectedProfiles.length; i++) {
        ...
        progressText.textContent = `Processing ${
          i + 1
        } of ${totalProfiles} profiles: ${profile.name}`;
        ...
        try {
          ...
          if (response) {
            if (typeof response === "string") {
              ...
              console.warn(
                "Bulk API response structure unclear for",
                profile.name,
                "- using default message"
              );
            }
          ...
          await this.openProfileAndSendMessage(
            profile.url,
            message,
            profile.name
          );
      ...
      Utils.showNotification(
        `Bulk processing completed! ${completedProfiles}/${totalProfiles} profiles processed successfully`,
        "success"
      );
    } catch (error) {
  ...
  async sendMessageToProfile(profileUrl, message, profileName, buttonElement) {
    try {
      const statusDiv =
        buttonElement.parentElement.querySelector(".message-status");
      ...
      statusText.textContent =
        "Navigating to LinkedIn profile in current tab...";
      ...
      buttonElement.disabled = false;
      const statusText =
        buttonElement.parentElement.querySelector(".status-text");
      statusText.textContent =
        "Failed to navigate to profile. Please try again.";
      statusText.style.color = "#dc3545";
    }
  },
  ...
  useSelectedMessages() {
    ...
    const profilesWithMessages = this.generatedMessages.filter(
      (item) => !item.message.error && item.selectedMessage
    );
    ...
    Utils.showNotification(
      `Selected ${finalMessages.length} messages for campaign`,
      "success"
    );
  ...
  skipToBulkSend() {
    ...
    this.showSendMessageInterface();
    Utils.showNotification(
      `Ready to process ${this.selectedProfiles.length} profiles`,
      "success"
    );
  },
};
...
const ProfileCollector = {
  start() {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      Utils.showNotification(
        "Starting single page profile collection...",
        "info"
      );
  ...
  startMultiPage() {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      Utils.showNotification(
        "Starting multi-page profile collection (1-4 pages)...",
        "info"
      );
      this.startMultiPageSearch(tab.id, {
        type: "multi-page",
        maxPages: 4,
        realTime: true,
      });
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          Utils.showNotification(
            "Real-time collection started! Profiles will appear as they are found.",
            "info"
          );
        } catch (error) {
          console.error("Message sending error:", error);
          Utils.showNotification(
            "Collection started. Profiles will appear as they are found.",
            "info"
          );
        }
      }, 1500);
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          Utils.showNotification(
            "Multi-page collection started! This may take a few minutes...",
            "info"
          );
        } catch (error) {
          console.error("Message sending error:", error);
          Utils.showNotification(
            "Multi-page collection started. Profiles will appear as they are found.",
            "info"
          );
        }
      }, 1500);
...
const NetworkManager = {
  openSearch() {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      chrome.tabs.update(
        tabs[0].id,
        { url: CONSTANTS.URLS.NETWORK_SEARCH },
        () => {
          Utils.showNotification(
            'LinkedIn network search opened. Use the filters to refine your search, then click "Start Collecting People"',
            "info"
          );
        }
      );
  ...
  browseConnections() {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      chrome.tabs.update(
        tabs[0].id,
        { url: CONSTANTS.URLS.CONNECTIONS },
        () => {
          Utils.showNotification(
            'LinkedIn connections page opened. You can browse and then click "Start Collecting People"',
            "info"
          );
        }
      );
  ...
  startCollecting() {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      Utils.showNotification(
        "Starting real-time profile collection...",
        "info"
      );

      if (
        tab.url.includes("search/results/people") &&
        tab.url.includes("network")
      ) {
        this.startSearch(tab.id, { type: "search", realTime: true });
      } else if (
        tab.url.includes("mynetwork") ||
        tab.url.includes("connections")
      ) {
  ...
  addProfilesDirectly(profiles) {
    ...
    ProfileManager.updateList();
    DOMCache.get("collected-number").textContent =
      AppState.collectedProfiles.length;
    Utils.showNotification(
      `Added ${validProfiles.length} profiles to campaign automatically`,
      "success"
    );
    const campaignModal = DOMCache.get("campaign-modal");
    if (campaignModal) {
...
const SalesNavigatorFloatingManager = {
  async launch() {
    try {
      ...
      setTimeout(() => {
        Utils.showNotification(
          "Sales Navigator opened! The floating UI will appear automatically.",
          "success"
        );
      }, 3000);
    } catch (error) {
...
const LinkedInSearchFloatingManager = {
  async launch() {
    try {
      ...
      setTimeout(() => {
        Utils.showNotification(
          "LinkedIn Search opened! The floating UI will appear automatically.",
          "success"
        );
      }, 3000);
    } catch (error) {
...
const ProfileURLModal = {
  show(profiles) {
    ...
    AppState.selectedProfiles = profiles.map((profile) => ({
      ...profile,
      selected: true,
    }));
                ...
                <div class="profile-pic">
                    ${
                      profilePicUrl
                        ...
                        : `<div style="width: 50px; height: 50px; border-radius: 50%; background: #0073b1; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 18px;">${cleanName
                            .charAt(0)
                            .toUpperCase()}</div>`
                    }
                </div>
  ...
  updateSelectedCount() {
    const selectedCount = AppState.selectedProfiles.filter(
      (p) => p.selected
    ).length;
  ...
  addSelected() {
    ...
    this.forceClose();
    Utils.showNotification(
      `Added ${profilesToAdd.length} profiles to campaign`,
      "success"
    );
    const campaignModal = DOMCache.get("campaign-modal");
    if (campaignModal) {
