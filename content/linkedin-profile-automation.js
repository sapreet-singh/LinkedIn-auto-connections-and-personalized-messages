...
class LinkedInProfileAutomation {
  ...
  checkForAutomationState() {
    ...
    console.log(
      "Checking for automation state on profile page:",
      !!automationState
    );
    ...
    if (automationState) {
      try {
        ...
        if (this.automationState.isAutomation) {
          console.log(
            "Profile automation detected for:",
            this.automationState.profileName
          );
          ...
          this.showAutomationIndicator(
            `LinkedIn Automation Active - Processing ${this.automationState.profileName}...`
          );
  ...
  async processConnection() {
    ...
    const totalProcessTime =
      this.typingDuration + this.sendDelay + this.postSendDelay + 15000;
    const timeoutId = setTimeout(() => {
      console.log("Profile automation timeout reached");
      this.showAutomationIndicator(
        "Automation timeout - returning to search",
        "error"
      );
      this.returnToSearchWithResult({
        success: false,
    ...
    try {
      console.log(
        "Starting profile connection for:",
        this.automationState.profileName
      );
      this.showAutomationIndicator(
        `Processing ${this.automationState.profileName}...`
      );
      ...
      this.showAutomationIndicator(
        "Connect button clicked, waiting for popup..."
      );
      ...
      if (!popupResult.success) {
        this.showAutomationIndicator(
          "Connection failed: " + popupResult.error,
          "error"
        );
        clearTimeout(timeoutId);
        setTimeout(() => {
      ...
      clearTimeout(timeoutId);
      this.showAutomationIndicator(
        `Connection sent successfully! Waiting ${
          this.postSendDelay / 1000
        } seconds before next profile...`,
        "success"
      );
      setTimeout(() => {
        this.returnToSearchWithResult({
  ...
  async findAndClickConnectButton() {
    return new Promise((resolve) => {
      ...
      const findButton = () => {
        ...
        this.showAutomationIndicator(
          `Looking for Connect button... (${attempts}/${maxAttempts})`
        );

        // Get selectors from configuration
        const primarySelectors = this.config.selectors?.connectButton
          ?.primary || [
        ...
        const fallbackSelectors = this.config.selectors?.connectButton
          ?.fallback || [
        ...
        if (!connectButton) {
          ...
          for (const button of allButtons) {
            const buttonTextElement = button.querySelector(
              ".artdeco-button__text"
            );
            if (
              buttonTextElement &&
              buttonTextElement.textContent.trim().toLowerCase() ===
                "connect" &&
              !button.disabled &&
              button.offsetParent !== null
            ) {
        ...
        if (attempts < maxAttempts) {
          ...
          console.log(
            "Connect button not found after",
            maxAttempts,
            "attempts"
          );
          resolve({
            success: false,
            error: "Connect button not found after multiple attempts",
          });
        }
      };
    ...
    try {
      ...
      const addNoteButton =
        ...
        document.querySelector(
          'button.artdeco-button--secondary .artdeco-button__text:contains("Add a note")'
        ) ||
      ...
      if (finalAddNoteButton) {
        ...
        console.log(
          'No "Add a note" button found, looking for textarea directly'
        );
      }
      ...
      if (noteTextarea && this.automationState.message) {
        ...
        await this.typeMessageWithDelay(
          noteTextarea,
          this.automationState.message
        );
        // Message typed successfully, now look for send button
        this.showAutomationIndicator(
          "Message typed successfully! Looking for Send button...",
          "success"
        );
      } else {
        ...
        for (const selector of fallbackSelectors) {
          try {
            ...
            for (const button of buttons) {
              ...
              if (
                textElement &&
                textElement.textContent.trim().toLowerCase() === "send"
              ) {
                sendButton = button;
                console.log(
                  "Found send button with fallback selector:",
                  selector
                );
                break;
              }
      ...
      if (sendButton) {
        ...
        console.log(
          "Send button not found, checking if connection was already sent"
        );
        return {
          success: true,
          message: "Connect button clicked (no send popup found)",
        };
      }
    } catch (error) {
      ...
      return {
        success: false,
        error: "Error handling popup: " + error.message,
      };
    }
  }
  ...
  fillConnectionMessage(popup, resolve) {
    try {
      ...
      resolve({
        success: false,
        error: `Error filling message: ${error.message}`,
      });
    }
  }
  ...
  returnToSearchWithResult(result) {
    ...
    console.log(
      ...
      this.automationState.currentUrl
    );
