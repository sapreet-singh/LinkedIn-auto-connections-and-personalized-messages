class LinkedInSearchFloatingUI {
  ...
  injectCSS() {
    ...
    link.onerror = () => {
      console.warn(
        "Failed to load external CSS, falling back to inline styles"
      );
  ...
  async loadHTMLTemplate() {
    try {
      const response = await fetch(
        chrome.runtime.getURL("content/linkedin-search-ui.html")
      );
  ...
  createFallbackUI() {
    console.error(
      "Failed to load external HTML template. Creating minimal fallback UI."
    );
    ...
    this.ui
      .querySelector(".linkedin-search-close")
  ...
  setupEventListeners() {
    ...
    connectBtn.addEventListener(
      "click",
      async () => await this.startConnecting()
    );
  ...
  startCollecting() {
    this.updateStatus(
      "status",
      this.config.messages?.status?.collecting || "Collecting profiles...",
      true
    );
  ...
  pauseCollecting() {
    this.updateStatus(
      "status",
      'Collection paused. Click "START COLLECTING" to resume.',
      false
    );
    this.isCollecting = false;
    ...
    for (const selector of selectors) {
      try {
        ...
        if (profileElements.length > 0) {
          console.log(
            `Found ${profileElements.length} profiles using selector: ${selector}`
          );
          break;
        }
      } catch (e) {
  ...
  extractProfileData(element) {
    try {
      ...
      const parentContainer =
        nameElement.closest("div[componentkey]") ||
      ...
      const imageElement =
        parentContainer.querySelector(`img[alt="${name}"]`) ||
      ...
      console.log(
        `Extracted profile: ${name} - ${title} at ${company} - ${location} (Status: ${connectionStatus})`
      );
  ...
  checkConnectionStatus(container) {
    try {
      ...
      for (const button of buttons) {
        ...
        if (
          (buttonText.includes("connect") || ariaLabel.includes("connect")) &&
      ...
      if (
        containerText.includes("pending") ||
        containerText.includes("invitation sent")
      ) {
        return "pending";
      }
  ...
  isDuplicateProfile(newProfile) {
    return this.collectedProfiles.some(
      (profile) =>
  ...
  updateProfilesList() {
    ...
    const emptyMessage =
      this.config.messages?.empty?.profiles ||
      'No connect profiles collected yet. Click "START COLLECTING" to begin.';

    if (this.collectedProfiles.length === 0) {
      profilesList.innerHTML = `<div class="empty-profiles">${emptyMessage}</div>`;
    } else {
      profilesList.innerHTML = this.collectedProfiles
        .map(
          (profile, index) => `
    ...
    navigator.clipboard
      ...
      .then(() => {
        ...
        notification.className =
          "linkedin-notification linkedin-notification-success";
  ...
  removeProfile(url) {
    this.collectedProfiles = this.collectedProfiles.filter(
  ...
  showNextButton() {
    const nextBtn = this.ui.querySelector("#start-connecting-btn");
    const buttonText =
      this.config.messages?.buttons?.startConnecting ||
      "Next: Start Connecting";
    ...
    const statusMessage = `${
      this.config.messages?.status?.collected ||
      "profiles collected. Ready to connect!"
    }`;
    this.updateStatus(
      "status",
      `Collected ${this.collectedProfiles.length} connect profiles. Ready to connect!`,
      false
    );
  }
  ...
  async showAutomationStarterUI() {
    ...
    const existingAutomationUI = document.querySelector(
    ...
    console.log(
      "Automation UI to append:",
      automationUI,
      "Type:",
      typeof automationUI,
      "Is Node:",
      automationUI instanceof Node
    );
  ...
  async restoreAutomationUI() {
    ...
    const uiAutomationState = sessionStorage.getItem(
      "linkedinUIAutomationState"
    );
    ...
    const restoredAutomationUI = document.querySelector(
      ".automation-starter-ui"
    );
    if (restoredAutomationUI) {
      const automationHeader =
        restoredAutomationUI.querySelector(".automation-header");
      if (automationHeader) {
  ...
  async createAutomationUI() {
    try {
      ...
      const automationTemplate = tempDiv.querySelector(
      ...
      if (!automationTemplate) {
        console.error(
          "Available elements in template:",
          tempDiv.querySelectorAll("[id]")
        );
      ...
      if (totalProfilesEl)
        totalProfilesEl.textContent = this.collectedProfiles.length;
      if (progressEl)
        progressEl.textContent = `${
          this.automationState?.currentIndex || 0
        } / ${this.collectedProfiles.length}`;
      if (profileUrlEl)
        profileUrlEl.textContent =
          this.collectedProfiles[this.automationState?.currentIndex || 0]
            ?.url || "None selected";
      ...
      console.warn(
        "Failed to load automation UI template, using fallback:",
        error
      );
  ...
  setupPromptState(automationUI) {
    ...
    const currentPromptText = automationUI.querySelector(
      "#current-prompt-text"
    );
    const customPromptTextarea = automationUI.querySelector("#custom-prompt");
    const startAutomationBtn = automationUI.querySelector(
      "#start-automation-btn"
    );

    console.log(
      "Setting up prompt state. Has saved prompt:",
    ...
    if (
      this.automationState &&
      this.automationState.promptSet &&
      this.automationState.customPrompt
    ) {
      ...
      if (currentPromptText)
        currentPromptText.textContent = this.automationState.customPrompt;
      ...
      if (startAutomationBtn) {
        startAutomationBtn.disabled = true;
        startAutomationBtn.textContent =
          "🚀 Start Automation (Set Prompt First)";
      }
    }
  }

  updateProfileListInUI(automationUI) {
    const profilesListElement = automationUI.querySelector(
    ...
    const profilesHTML = this.collectedProfiles
      .map((profile, index) => {
        let status = "⏳"; // Waiting
  ...
  processConnectionRequests() {
    ...
    sendConnectCount.textContent = Math.floor(
      this.collectedProfiles.length * sendRatio
    );
    FailedConnectCount.textContent = Math.floor(
      this.collectedProfiles.length * FailedRatio
    );
  }
  ...
  setupAutomationEventListeners(automationUI) {
    ...
    const pauseAutomationBtn = automationUI.querySelector(
      "#pause-automation-btn"
    );
    const stopAutomationBtn = automationUI.querySelector(
      "#stop-automation-btn"
    );
    const customPromptTextarea = automationUI.querySelector("#custom-prompt");
    ...
    if (closeBtn) {
      closeBtn.addEventListener("click", () => {
        if (this.automationState.isRunning) {
          if (
            confirm("Automation is running. Are you sure you want to close?")
          ) {
            this.stopAutomation();
            automationUI.remove();
            if (this.ui) {
  ...
  updateAutomationProgress() {
    ...
    const currentProfileUrlElement = document.querySelector(
      "#current-profile-url"
    );
    const profilesListElement = document.querySelector(
      "#profiles-list-automation"
    );

    // Use currentIndex or currentProfileIndex, whichever is available
    const currentIndex =
      this.automationState?.currentIndex ??
      this.automationState?.currentProfileIndex ??
      0;
    const totalProfiles =
      this.automationState?.totalProfiles ??
      this.collectedProfiles?.length ??
      0;
    ...
    if (progressFillElement) {
      const percentage =
        totalProfiles > 0 ? (currentIndex / totalProfiles) * 100 : 0;
  ...
  updateAutomationProfilesList(container) {
    ...
    container.innerHTML = this.collectedProfiles
      .map((profile, index) => {
        let status = "Waiting";
        let statusClass = "waiting";
        let statusIcon = "⏳";
  ...
  clearProfiles() {
    ...
    this.updateStatus(
      "status",
      this.config.messages?.status?.ready ||
        "Ready to start collecting connect profiles only",
      false
    );
  }
  ...
  async startAutomationProcess(automationUI) {
    ...
    this.showNotification(
      "Starting same-tab automation. The popup will be preserved during navigation.",
      "info"
    );
  ...
  async processProfilesSequentially() {
    for (
      let i = this.automationState.currentProfileIndex;
      i < this.collectedProfiles.length;
      i++
    ) {
      if (!this.automationState.isRunning) {
      ...
      console.log(
        `Processing profile ${i + 1}/${this.collectedProfiles.length}: ${
          profile.name
        }`
      );
      ...
      try {
        ...
        const messageData = await this.generateMessageForProfile(
          profile.url,
          this.automationState.customPrompt
        );
        ...
        console.log(
          `Profile ${i + 1}/${this.collectedProfiles.length} - Opening: ${
            profile.url
          }`
        );
        const result = await this.openProfileAndConnect(
          profile.url,
          messageData.message,
          profile.name
        );
        ...
        console.log(
          `Automation still running: ${this.automationState.isRunning}`
        );
        ...
        if (result.success) {
          ...
          console.log(
            `Successfully processed profile ${i + 1}: ${profile.name}`
          );
        } else {
          ...
          console.log(
            `Failed to process profile ${i + 1}: ${profile.name} - ${
              result.error
            }`
          );
        ...
        if (i < this.collectedProfiles.length - 1) {
          // Don't wait after the last profile
          console.log(
            `Waiting before next profile... (${i + 1}/${
              this.collectedProfiles.length
            })`
          );
          console.log(
            `Next profile will be: ${this.collectedProfiles[i + 1]?.name}`
          );
  ...
  async continueAutomationAfterProfile() {
    ...
    console.log(
      ...
      this.collectedProfiles.length,
      "Last index:",
      this.collectedProfiles.length - 1
    );
    ...
    if (currentIndex + 1 < this.collectedProfiles.length) {
      ...
      console.log(
        `Moving from profile ${currentIndex} to profile ${nextIndex}`
      );
      ...
      console.log(
        `Continuing with profile ${nextIndex + 1}/${
          this.collectedProfiles.length
        }`
      );
  ...
  async processNextProfileInSequence(startIndex) {
    ...
    if (startIndex >= this.collectedProfiles.length) {
      console.log(
        `Start index ${startIndex} is out of bounds (total profiles: ${this.collectedProfiles.length}), completing automation`
      );
      this.automationState.isRunning = false;
      this.updateAutomationProgress();
      this.closeAutomationUI();
  ...
  async generateMessageForProfile(profileUrl, customPrompt) {
    try {
      ...
      const message =
        ...
        "Hi, I'm Ishu, a full-stack developer with expertise in .NET, Angular, and React. I'd love to support your development needs. Let's connect and explore how I can add value to your team.";
      return { message };
    } catch (error) {
  ...
  async openProfileAndConnect(profileUrl, message, profileName) {
    return new Promise((resolve) => {
      ...
      sessionStorage.setItem(
        "linkedinUIAutomationState",
        JSON.stringify(uiAutomationState)
      );
  ...
  showNotification(message, type = "info") {
    ...
    const existingNotifications = document.querySelectorAll(
      ".linkedin-automation-notification"
    );
  ...
  showCompletionNotification() {
    ...
    const notification = document.createElement("div");
    notification.className =
      "linkedin-automation-notification linkedin-notification linkedin-notification-success";
    notification.innerHTML = `
...
function initLinkedInSearchUI() {
  if (window.location.href.includes("linkedin.com/search")) {
      ...
      try {
        ...
        if (window["linkedInSearchUI"] && window.automationResolve) {
          ...
          console.log(
            ...
            window["linkedInSearchUI"].automationState
          );
            ...
            setTimeout(async () => {
              ...
              if (window.automationResolve) {
                ...
                if (window["linkedInSearchUI"].automationState?.isRunning) {
                  console.log(
                    "Continuing automation after profile processing..."
                  );
                  setTimeout(() => {
          ...
          window.automationResolve = null;
          console.log(
            "Automation result processed successfully, automation should continue..."
          );
    ...
    if (
      ...
      console.log(
        "UI already exists or automation result being processed, skipping creation"
      );
    }
  }
}
...
function initializeLinkedInAutomation() {
  console.log(
    "Initializing LinkedIn automation for URL:",
  ...
  if (window.location.href.includes("linkedin.com/search")) {
    ...
    console.log(
      "Not a LinkedIn search page, skipping search UI initialization"
    );
  }
